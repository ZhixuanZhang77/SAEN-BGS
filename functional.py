...
# from operationNN import OperationFC,OperationConv2d,operationConv2dFunc,operatioFC1dFunc
########### using in SpikeVAE ########################
...
class ConvTranspose2dIF_v2(torch.autograd.Function):

    @staticmethod
    def forward(ctx, spike_in, features_in, weight, device=torch.device(SETTINGS.training.device), bias=None, stride=1, padding=0,output_padding=0, pooling=1,training=False):
        """
        args:
            spike_in: (N, T, in_channels, iH, iW)
            features_in: (N, in_channels, iH, iW)
        ...
        # spike_mask = torch.zeros_like(pot_aggregate,device=device).float()
        if training:
            add_spike, _ = ZeroExpandInput_CNN.apply(features_in/T, T)
            # spike_in += add_spike
    ...
    def backward(ctx, grad_spike_out, grad_spike_count_out):
        ...
        grad_spike_in, grad_weight, grad_device, grad_bias, grad_stride, grad_padding, grad_output_padding,grad_pooling,grad_training = None, \
                None, None, None, None, None, None, None, None

        return grad_spike_in, grad_spike_count_out, grad_weight, grad_device, grad_bias, \
                grad_stride, grad_padding, grad_output_padding,grad_pooling,grad_training



###############################
# class OperationFCIF(torch.autograd.Function):
#
# 	@staticmethod
...
# 		# pot_in =  ( weight*spike_in[:,:, None, :]).sin().sum (3)#spike_in.matmul(weight.t())
...
#
# 		N, T, _ = spike_in.shape
# 		pot_aggregate = operatioFC1dFunc(torch.zeros_like(spike_in[:,0,:]),weight,bias) #bias.repeat(N, 1)bias.repeat(N, 1)
# 		_,  outH = pot_aggregate.shape
# 		spike_out = torch.zeros(N, T,  outH,  device=device)
# 		spike_mask = torch.zeros_like(pot_aggregate, device=device).float()
#
# 		# Iterate over simulation time steps to determine output spike trains
# 		for t in range(T):
# 			pot_aggregate += operatioFC1dFunc(spike_in[:,t,:],weight,None)
# 			bool_spike = torch.ge(pot_aggregate, 1.0).float()
#
# 			bool_spike *= (1 - spike_mask)
#
...
#
# class OperationConv2dIF(torch.autograd.Function):
#
# 	@staticmethod
...
# 		pot_aggregate = F.max_pool2d(operationConv2dFunc(torch.zeros_like(spike_in[:,0,:,:,:]), weight, bias, stride, padding), pooling) # init the membrane potential with the bias
# 		_, _, outH, outW = pot_aggregate.shape
# 		spike_out = torch.zeros(N, T, out_channels, outH, outW, device=device)
# 		spike_mask = torch.zeros_like(pot_aggregate,device=device).float()
#
# 		# Iterate over simulation time steps to determine output spike trains
# 		for t in range(T):
# 			pot_aggregate += F.max_pool2d(operationConv2dFunc(spike_in[:,t,:,:,:], weight, None, stride, padding), pooling)
# 			bool_spike = torch.ge(pot_aggregate, 1.0).float()
#
# 			bool_spike *=(1-spike_mask)
#
...
class Conv2dIF_v2(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, spike_in, features_in, weight, device=torch.device(SETTINGS.training.device), bias=None, stride=1, padding=0, pooling=1,training=False):
        """
        args:
            spike_in: (N, T, in_channels, iH, iW)
            features_in: (N, in_channels, iH, iW)
        ...
        for t in range(T):
            pot_aggregate += F.max_pool2d(F.conv2d(spike_in[:,t,:,:,:], weight, None, stride, padding), pooling)
            if training:
                pot_aggregate += F.sigmoid(features_in)/2*T#[:,t,:,:,:]
            bool_spike = torch.ge(pot_aggregate, 1.0).float()
    ...
    def backward(ctx, grad_spike_out, grad_spike_count_out):
        ...
        grad_spike_in, grad_weight, grad_device, grad_bias, grad_stride, grad_padding, grad_pooling,grad_training = None, \
                None, None, None, None, None, None, None

        return grad_spike_in, grad_spike_count_out, grad_weight, grad_device, grad_bias, \
                grad_stride, grad_padding, grad_pooling,grad_training
...
class Conv1dIF(torch.autograd.Function):
    ...
    def forward(ctx, spike_in, features_in, weight, device=torch.device('cuda'), bias=None,\
        ...
        pot_aggregate = F.conv1d(torch.zeros_like(spike_in[:,0,:,:]), weight, bias, stride, padding) # init the membrane potential with the bias
        ...
        # Iterate over simulation time steps to determine output spike trains
        for t in range(T):
            pot_aggregate += F.conv1d(spike_in[:,t,:,:], weight, None, stride, padding)
            bool_spike = torch.ge(pot_aggregate, 1.0).float()
...
#
#	@staticmethod
#	def forward(ctx, spike_in, features_in, weight, device=torch.device('cuda'), bias=None, stride=1, padding=0):
#		"""
#		args:
#			spike_in: (N, T, in_channels, iH, iW)
#			features_in: (N, in_channels, iH, iW)
...
#		N, T, in_channels, iH, iW = spike_in.shape
#		out_channels, in_channels, kH, kW = weight.shape
#		pot_aggregate = F.conv2d(spike_in[:,0,:,:,:], weight, bias, stride, padding) # init the membrane potential with the bias
#		_, _, outH, outW = pot_aggregate.shape
#		spike_out = torch.zeros(N, T, out_channels, outH, outW, device=device)
#
#		# Iterate over simulation time steps to determine output spike trains
#		for t in range(T):
#			pot_aggregate += F.conv2d(spike_in[:,t,:,:,:], weight, None, stride, padding)
